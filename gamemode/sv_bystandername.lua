...
local function addPart(name, sex)
	...
	tab.name = name
	tab.sex = sex
	table_insert(GM.BystanderNameParts, tab)
end
...
function GM:AddBystanderNamePart(name, sex)
	...
	table_insert(self.BystanderNameParts, tab)
end
...
function ENTITY:GenerateBystanderName()
...
function ENTITY:SetBystanderName(name)
	self:SetNWString("bystanderName", name)
	self.BystanderName = name
end
